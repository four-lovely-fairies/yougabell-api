...
처음부터 모듈이 꽤 많아질 걸 알고 있었다. 사용자, 아이, 놀이, 마일스톤, 마음 배터리, 챗봇, 주간 리포트… 각각이 컨트롤러 + 서비스 + DTO 묶음으로 떨어지는데, 이걸 손으로 묶기 시작하면 금방 폴더 구조가 제멋대로 된다.
...
| `missions`       | 놀이 카탈로그, 실행 추적, 피드백 수집(아이 반응 / 부모 에너지)                    |
...
| `weekly-reports` | 주간 리포트 배치 — 놀이·배터리·키워드를 LLM으로 합성                              |
...
- **`csv-parse`** — 운영자 시트에서 받은 놀이·마일스톤 시드를 일괄 적재. 시트마다 컬럼 순서가 달라서 시트별 파서를 따로 둔다.
