# Repo Strategy — Youth (working-mom-dad)
...
| 항목 | 결정 | 비고 |
|---|---|---|
| 레포 구조 | **완전 분리 (4 레포)** | 모노레포 X. `youth-corp` org 산하 |
...
| GitHub 레포 | 역할 | 스택 | 호스팅 |
|---|---|---|---|
| [`youth-corp/working-mom-dad-api`](https://github.com/youth-corp/working-mom-dad-api) | 도메인 API · 챗봇 · LLM 게이트웨이 | NestJS, Prisma, TS | TBD |
| [`youth-corp/working-mom-dad-web`](https://github.com/youth-corp/working-mom-dad-web) | 사용자용 웹 (Expo WebView 타깃) | Next.js 16, Tailwind, TS | Vercel |
| [`youth-corp/working-mom-dad-admin`](https://github.com/youth-corp/working-mom-dad-admin) | 운영자 CMS (콘텐츠/미션/리포트) | Next.js 16, Tailwind, TS | Vercel |
| [`youth-corp/working-mom-dad-mobile`](https://github.com/youth-corp/working-mom-dad-mobile) | RN 셸 (푸시·인증·WebView 컨테이너) | Expo, TS | EAS |
...
flowchart TB
    ...
    API[working-mom-dad-api<br/>NestJS + Prisma]
    WEB[working-mom-dad-web<br/>Next.js]
    ADMIN[working-mom-dad-admin<br/>Next.js]
    APP[working-mom-dad-mobile<br/>Expo RN]
...
| 소비처 | 받아오는 방식 |
|---|---|
| `working-mom-dad-api` | Prisma 본가. `schema.prisma` + `prisma generate` |
| `working-mom-dad-web` | API에서 OpenAPI 스펙 자동 export → 클라이언트에서 codegen (`openapi-typescript` 또는 `orval`). **Prisma 직접 import 안 함.** |
| `working-mom-dad-admin` | 동일. OpenAPI 코드젠. (단, admin은 추후 Prisma direct 검토 가능 — DB에 직접 붙는 운영 화면이 필요하면) |
| `working-mom-dad-mobile` | OpenAPI 코드젠. Prisma는 RN 런타임 미지원이므로 **DB 직접 접근 X**. |
...
### Phase 1 — 인프라 셋업 (선행)
- [x] `working-mom-dad-moblie` → `working-mom-dad-mobile` 레포명 변경
...
- [ ] `working-mom-dad-api` 클론 → NestJS + Prisma 초기화
...
- [ ] `working-mom-dad-web` — Next.js 16, Tailwind, OpenAPI 코드젠
- [ ] `working-mom-dad-admin` — Next.js 16, Tailwind, OpenAPI 코드젠, 인증 게이트
- [ ] `working-mom-dad-mobile` — Expo, WebView 셸, OpenAPI 코드젠
